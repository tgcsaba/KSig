...
```
Also, it seems that CuPy ships CUDA specific versions with the naming convention `cupy-cudaXX`, which are not properly recognized by setuptools as being a version of CuPy. This arises in Google Colab environments for example, and there it is recommended to use pip install with the `--no-deps` option to avoid reinstalling.
